    ...
    return (
        <div className={cn("md:p-8 max-md:p-5 bg-white rounded-2xl shadow-xl border border-gray-100", className)}>
            ...
            <ul className="flex flex-col gap-y-4 h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {Array.isArray(tasks) &&
                    tasks.map((task) => (
                        <li key={task.id} className={`p-5 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 ${getStatusColor(task.status)}`}>
                            <div className="flex max-md:flex-col max-md:gap-4 md:justify-between md:items-start">
                                ...
                                <div className="flex max-md:flex-col md:items-center gap-3 md:ml-4">
                                    ...
                                    >
                                        <span className="text-sm">{dropdownOpen[task.id] ? "Hide" : "Show"} Details</span>
                                        {dropdownOpen[task.id] ? <FaCaretUp className="w-5 h-5" /> : <FaCaretDown className="w-5 h-5" />}
                                    </button>
