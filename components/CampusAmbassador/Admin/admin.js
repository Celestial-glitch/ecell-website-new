    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            {submissions.map((s) => (
                ...
                >
                    <Stack spacing={1}>
                        <Typography fontWeight={600}>{s.taskTitle}</Typography>
                        ...
                        <Typography variant="body2">
                            Submitted Link:{" "}
                            <a href={s.driveLink} target="_blank" rel="noreferrer" style={{ color: "#1976d2" }}>
                                Open Drive Link
                            </a>
                        </Typography>

                        <Chip label={s.status.toUpperCase()} color={s.status === "accepted" ? "success" : s.status === "rejected" ? "error" : "warning"} sx={{ width: "fit-content" }} />

                        <Stack direction="row" spacing={2} mt={1}>
                            <Button variant="contained" color="success" disabled={s.status === "accepted"} onClick={() => updateStatus(s.id, "accepted")}>
                                Accept
                            </Button>

                            <Button variant="contained" color="error" disabled={s.status === "rejected"} onClick={() => updateStatus(s.id, "rejected")}>
                                Reject
                            </Button>
                        </Stack>
                    </Stack>
